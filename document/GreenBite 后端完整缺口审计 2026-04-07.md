...
### 4.2 菜品接口缺少展示图片字段与文件上传能力

当前前端已经有“菜品展示图片”的实际需求，但后端的数据模型和接口里没有对应字段，例如：
...
- 前端只能把图片 URL 存在本地缓存
- 当前前端即使改成“选择文件”，也只能转成本地 data URL 临时保存
- 不同浏览器、不同设备之间不会同步
- 员工修改图片不是真正的后端持久化

建议：

- 在 `meals` 表中新增图片字段，建议名称统一为 `image_url`
- 在以下接口中补充该字段：
  - `GET /meals`
  - `GET /meals/{id}`
- 增加图片上传接口，建议二选一：
...
### 4.5 `GET /recommendations` 的返回信息不够丰富
...
### 4.6 Dashboard 的 `topMeals` SQL 语义可能不准确
